...
namespace mrs_uav_trajectory_generation
{
  ...
  void MrsTrajectoryGeneration::onInit()
  {
    ...
    sh_tracker_cmd_ = mrs_lib::SubscribeHandler<mrs_msgs::TrackerCommand>(shopts, "tracker_cmd_in", &MrsTrajectoryGeneration::callbackTrackerCmd, this);
  ...
  std::tuple<std::optional<mrs_msgs::TrackerCommand>, bool, int> MrsTrajectoryGeneration::prepareInitialCondition(const ros::Time path_time)
  {
    ...
    if (path_time_offset > 0.2)
    {
      ...
      } else
      {
        ...
        full_state.position = tracker_cmd->full_state_prediction.position[path_sample_offset];
        full_state.velocity = tracker_cmd->full_state_prediction.velocity[path_sample_offset];
        full_state.acceleration = tracker_cmd->full_state_prediction.acceleration[path_sample_offset];
        full_state.jerk = tracker_cmd->full_state_prediction.jerk[path_sample_offset];

        full_state.heading = tracker_cmd->full_state_prediction.heading[path_sample_offset];
        full_state.heading_rate = tracker_cmd->full_state_prediction.heading_rate[path_sample_offset];
    ...
    for (size_t i = 0; i < waypoints.size(); i++)
    {
      ...
      if (i == 0)
      {
        ...
        if (initial_state)
        {
          ...
          vertex.addConstraint(eth_trajectory_generation::derivative_order::ACCELERATION,
                               Eigen::Vector4d(initial_state->acceleration.x, initial_state->acceleration.y, initial_state->acceleration.z,
                                               initial_state->heading_acceleration));
    ...
    if (override_constraints_)
    {
      ...
      if (initial_state)
      {
        can_change = (hypot(initial_state->velocity.x, initial_state->velocity.y) < override_max_velocity_horizontal_)
                     ...
                     && (fabs(initial_state->acceleration.z) < override_max_acceleration_vertical_)
  ...
  void MrsTrajectoryGeneration::callbackTrackerCmd(const mrs_msgs::TrackerCommand::ConstPtr msg)
  {
    ...
    ROS_INFO_ONCE("[MrsTrajectoryGeneration]: got tracker cmd");
