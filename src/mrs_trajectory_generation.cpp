...
#include <mrs_msgs/PathWithVelocity.h>
...
struct Waypoint_t
{
  Waypoint_t(const Eigen::Vector4d coords) : coords(coords) {};
  Eigen::Vector4d coords;
  bool stop_at = false;
  bool enforce_velocity = false;
...
namespace mrs_uav_trajectory_generation
{
  ...
  private:
    ...
    void callbackPath(const mrs_msgs::PathWithVelocity::ConstPtr& msg);
  ...
  std::vector<Waypoint_t> MrsTrajectoryGeneration::preprocessPath(const std::vector<Waypoint_t>& waypoints_in)
  {
    ...
    for (size_t i = 0; i < waypoints_in.size(); i++)
    {
      ...
      Waypoint_t wp(Eigen::Vector4d(x, y, z, heading));
      wp.stop_at = waypoints_in.at(i).stop_at;
      wp.enforce_velocity = waypoints_in.at(i).enforce_velocity;
      wp.velocity = waypoints_in.at(i).velocity;
      ...
      if (_trajectory_max_segment_deviation_enabled_ && !safe)
      {
        ...
        for (; waypoint < waypoints.end() - 1; waypoint++)
        {
          ...
          if (!(*safeness))
          {
            ...
            {
              const Waypoint_t midpoint2 = interpolatePoint(*waypoint, *(waypoint + 1), 0.5);
                                                                                                    ...
                                                                                                    const double& sampling_dt, const bool& relax_heading)
  {
    ...
    for (size_t i = 0; i < waypoints.size(); i++)
    {
      ...
      {  // mid points
        ...
        else if (waypoints.at(i).enforce_velocity)
        {
          vertex.addConstraint(eth_trajectory_generation::derivative_order::VELOCITY, waypoints.at(i).velocity);
        }
      }
    ...
    segment_times_baca = estimateSegmentTimesBaca(vertices, v_max_horizontal, v_max_vertical, a_max_horizontal, a_max_vertical, j_max_horizontal,
                                                  j_max_vertical, v_max_heading, a_max_heading);
                                                                                                            ...
                                                                                                            const bool& relax_heading)
  {
    ...
    for (size_t i = 0; i < waypoints.size(); i++)
    {
      ...
      vertex.addConstraint(eth_trajectory_generation::derivative_order::POSITION, Eigen::Vector4d(x, y, z, heading));
      if (waypoints.at(i).enforce_velocity)
        vertex.addConstraint(eth_trajectory_generation::derivative_order::VELOCITY, waypoints.at(i).velocity);
    ...
    for (size_t i = 0; i < waypoints.size() - 1; i++)
    {
      ...
      for (int j = 0; j <= n_samples; j++)
      {
        ...
        const Waypoint_t point = interpolatePoint(waypoints[i], waypoints[i + 1], j * interp_step);
                                                                          ...
                                                                          const std::vector<Waypoint_t>& waypoints)
  {
    ...
    for (size_t i = 0; i < trajectory.size() - 1; i++)
    {
      ...
      // segment end
      const vec3_t segment_end = vec3_t(waypoints.at(last_segment_start + 1).coords[0], waypoints.at(last_segment_start + 1).coords[1],
                                        waypoints.at(last_segment_start + 1).coords[2]);
  ...
  {
    const Eigen::Vector4d diff = b.coords - a.coords;
    const Eigen::Vector4d interp(
      a.coords.x() + coeff * diff.x(),
      a.coords.y() + coeff * diff.y(),
      a.coords.z() + coeff * diff.z(),
      radians::interp(a.coords.w(), b.coords.w(), coeff)
      );
    return Waypoint_t(interp);
  }
  ...
  void MrsTrajectoryGeneration::callbackPath(const mrs_msgs::PathWithVelocity::ConstPtr& msg)
  {
    ...
    for (size_t i = 0; i < msg->points.size(); i++)
    {
      ...
      const double x = msg->points[i].position.x;
      const double y = msg->points[i].position.y;
      const double z = msg->points[i].position.z;
      const double heading = msg->points[i].heading;

      const bool enforce_velocity = msg->points[i].enforce_velocity;
      const double v_x = msg->points[i].velocity.x;
      const double v_y = msg->points[i].velocity.y;
      const double v_z = msg->points[i].velocity.z;

      Waypoint_t wp(Eigen::Vector4d(x, y, z, heading));
      wp.stop_at = stop_at_waypoints_;
      wp.enforce_velocity = enforce_velocity;
      wp.velocity = Eigen::Vector4d(v_x, v_y, v_z, 0.0);
