...
inline std::ostream &operator<<(std::ostream &stream, const OptimizationInfo &val) {
  ...
  for (const std::pair<const int, Extremum> &m : val.maxima) {
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeMellingerOuterLoop() {
  ...
  if (optimization_parameters_.print_debug_info_time_allocation) {
    ...
    std::for_each(original_segment_times.cbegin(), original_segment_times.cend(), [](double c) { std::cout << c << " "; });
    std::cout << std::endl;
    std::cout << "[MEL RELATIVE Solution]: ";
    std::for_each(relative_segment_times.cbegin(), relative_segment_times.cend(), [](double c) { std::cout << c << " "; });
    std::cout << std::endl;
    std::cout << "[MEL          Solution]: ";
    std::for_each(scaled_segment_times.cbegin(), scaled_segment_times.cend(), [](double c) { std::cout << c << " "; });
    std::cout << std::endl;
