...
inline double MagnitudeOfGravity(const double height, const double latitude_radians) {
  ...
  return kGravity_0 * ((1 + kGravity_a * sin_squared_latitude - kGravity_b * sin_squared_twice_latitude) - kGravity_c * height);
}
...
inline bool vectorFromSkewMatrix(const Eigen::Matrix3d& vec_skew, Eigen::Vector3d* vec) {
  ...
  if ((vec_skew + vec_skew.transpose()).norm() < kSmallValueCheck) {
    *vec << vec_skew(2, 1), vec_skew(0, 2), vec_skew(1, 0);
    ...
    std::cerr << "[eth_mav_msgs] Matrix is not skew-symmetric." << std::endl;
...
inline bool isRotationMatrix(const Eigen::Matrix3d& mat) {
  ...
  if (mat.determinant() - 1.0 > kSmallValueCheck) {
    std::cerr << "[eth_mav_msgs::common] Rotation matrix requirement violated (det(R) = 1)" << std::endl;
    return false;
  }
  return true;
}
...
inline void matrixFromRotationVector(const Eigen::Vector3d& vec, Eigen::Matrix3d* mat) {
  ...
  *mat = Eigen::Matrix3d::Identity() + vec_skew_norm * std::sin(r_norm) + vec_skew_norm * vec_skew_norm * (1 - std::cos(r_norm));
}

//}

/* vectorFromRotationMatrix() //{ */
...
inline bool vectorFromRotationMatrix(const Eigen::Matrix3d& mat, Eigen::Vector3d* vec) {
  ...
  if (!isRotationMatrix(mat)) {
    std::cerr << "[eth_mav_msgs::common] Not a rotation matrix." << std::endl;
    return false;
  }
...
inline Eigen::Vector3d omegaFromRotationVector(const Eigen::Vector3d& rot_vec, const Eigen::Vector3d& rot_vec_vel) {
  ...
  Eigen::Matrix3d W;
  W = Eigen::MatrixXd::Identity(3, 3) + phi_skew * (1 - std::cos(phi)) * phi_2_inv + phi_skew * phi_skew * (phi - std::sin(phi)) * phi_3_inv;
