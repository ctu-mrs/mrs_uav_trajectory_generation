...
Tester::Tester() {
  ...
  sch_path_ = mrs_lib::ServiceClientHandler<mrs_msgs::PathSrv>(nh_, "/" + uav_name + "/trajectory_generation/path");
...
bool Tester::test() {
  ...
  mrs_msgs::PathSrv path_cmd;

  path_cmd.request.path.fly_now     = true;
  path_cmd.request.path.use_heading = true;

  mrs_msgs::Reference point;

  point.position.x = 0;
  point.position.y = 0;
  point.position.z = 5.5;
  point.heading    = 2.2;

  path_cmd.request.path.points.push_back(point);

  {

    ROS_INFO("[%s]: calling path", ros::this_node::getName().c_str());

    {
      bool service_call = sch_path_.call(path_cmd);

      if (!service_call || !path_cmd.response.success) {
        ROS_ERROR("[%s]: path service call failed", ros::this_node::getName().c_str());
        return false;
      }
    }
  }
